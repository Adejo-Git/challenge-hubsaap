...
export function normalizePermissions(
  permissions: PermissionKey | PermissionKey[] | undefined | null
): PermissionKey[] {
  if (!permissions) {
    return [];
  }
...
export function normalizeRoles(
  roles: RoleKey | RoleKey[] | undefined | null
): RoleKey[] {
  if (!roles) {
    return [];
  }
 ...
 */
export function isValidPermissionKey(key: PermissionKey | undefined | null): boolean {
 ...
 * @returns true se válida, false caso contrário
 */
export function isValidRoleKey(key: RoleKey | undefined | null): boolean {
  if (!key || typeof key !== 'string') {
    return false;
  }
...
export function filterValidPermissions(
  keys: PermissionKey[] | undefined | null
): PermissionKey[] {
  if (!keys) {
    return [];
  }
 ...
 * @returns Array filtrado contendo somente keys válidas
 */
export function filterValidRoles(
  keys: RoleKey[] | undefined | null
): RoleKey[] {
  if (!keys) {
    return [];
  }
 ...
 */
export function mergeGrants(...grantsSets: Set<PermissionKey>[]): Set<PermissionKey> {
 ...
 */
export function rolesEqual(role1: RoleKey, role2: RoleKey): boolean {
  if (!role1 || !role2) {
    return false;
  }
 ...
 */
export function hasRoleInList(role: RoleKey, roleList: RoleKey[]): boolean {
 ...
 */
export function simpleCacheKey(...input: string[]): string {
