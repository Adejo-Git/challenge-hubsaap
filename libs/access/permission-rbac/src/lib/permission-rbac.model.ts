...
export type ExplainCode =
  ...
  | 'missing-permission'       // Permissão não está nos grants
  | 'missing-role'             // Role não está na sessão
  | 'invalid-key'              // PermissionKey ou RoleKey inválida(o)
  | 'config-error'             // Configuração do RBAC inválida
  | 'strategy-unsupported'     // Estratégia não suportada
  | 'no-session'               // Sessão não disponível/válida
  | 'cache-miss';              // Cache miss (informacional)
