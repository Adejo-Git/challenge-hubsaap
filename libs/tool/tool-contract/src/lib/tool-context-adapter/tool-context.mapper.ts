 ...
 */
export function mapToToolSession(session: ISession | null): ToolSession | null {
  if (!session?.user) {
    return null;
  }
 ...
 */
export function mapToToolContext(
 ...
 */
export function mapToToolRuntimeCapabilities(
  enabledFeatures: string[],
  allowedActions: string[],
  isToolEnabled: boolean,
  disabledReason?: string
): ToolRuntimeCapabilities {
  const capabilities: ToolRuntimeCapabilities = {
...
export function detectChangeType(
  ...
  current: ToolContext
): 'tenant' | 'client' | 'project' | 'session' | 'full' {
  if (!previous) {
    return 'full';
  }
