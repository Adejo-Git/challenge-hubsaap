...
export function sanitize(details: unknown): ErrorDetailsSafe {
  if (details == null) return details as ErrorDetailsSafe;
  ...
  if (typeof details === 'string') {
    ...
    copy = Array.isArray(details) ? [...details] : { ...(details as Record<string, unknown>) };
  } else {
    return details as ErrorDetailsSafe;
  }

  const stripSensitive = (obj: unknown, depth: number): ErrorDetailsSafe => {
    if (!obj || typeof obj !== 'object') return obj as ErrorDetailsSafe;
    if (depth >= SANITIZE_MAX_DEPTH) return '[TRUNCATED_DEPTH]';
  ...
  return sanitized as ErrorDetailsSafe;
}

export function buildUserMessage(code: ErrorCode | string): string {
...
export function mapErrorCodeToHttpStatus(code: ErrorCode | string): number {
  switch (code) {
    case ErrorCode.AUTH_EXPIRED:
