 ...
 */
export function buildPermissionMap(config) {
    const map = new Map();
 ...
 */
export function resolveRolePermissions(roleKey, map, _visited = new Set()) {
    ...
    const inheritedGrants = roleDef.extends.map((extendedRole) => resolveRolePermissions(extendedRole, map, _visited));
 ...
 */
export function resolveMultiRolePermissions(roles, map) {
 ...
 */
export function validatePermissionMap(map) {
    const errors = [];
    map.forEach((roleDef, roleKey) => {
        ...
        if (roleDef.extends) {
            for (const extendedRole of roleDef.extends) {
                ...
                if (!map.has(normalizedExtended)) {
                    errors.push(`Role "${roleKey}" extends "${extendedRole}", que não existe no mapa.`);
                }
            }
        }
        ...
        if ((!roleDef.permissions || roleDef.permissions.length === 0) &&
            ...
            errors.push(`Role "${roleKey}" não define permissões diretas nem herança (extends).`);
        }
    });
    return errors;
}
 ...
 */
export function detectCycles(map) {
    const errors = [];
    map.forEach((roleDef, roleKey) => {
        if (!roleDef.extends) {
            return;
        ...
        for (const extendedRole of roleDef.extends) {
            ...
            if (extendedDef && extendedDef.extends) {
                ...
                if (extendedDef.extends.some((e) => e.trim().toUpperCase() === roleKey.toUpperCase())) {
                    errors.push(`Ciclo detectado: "${roleKey}" extends "${extendedRole}", e "${extendedRole}" extends "${roleKey}".`);
                }
            }
        }
    });
 ...
 */
export function createDefaultPermissionMap() {
    const config = {
        roles: [
            {
                key: 'VIEWER',
