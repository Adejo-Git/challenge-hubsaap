 ...
 */
export function validateToolKey(toolKey) {
    const errors = [];
    const warnings = [];
    const toolKeyStr = toolKey;
 ...
 */
export function validatePermissionKey(permissionKey, toolKey) {
    const errors = [];
    const warnings = [];
    const permKeyStr = permissionKey;
    const toolKeyStr = toolKey;
 ...
 * Formato esperado: ${toolKey}.
 */
export function validateFeatureKey(featureKey, toolKey) {
    const errors = [];
    const warnings = [];
    const featKeyStr = featureKey;
    const toolKeyStr = toolKey;
 ...
 */
function validateDeepLinkPermissions(deepLinkId, requiredPermissions, declaredPermissions) {
    const warnings = [];
 ...
 */
function validateDeepLinkFeatures(deepLinkId, requiredFeatures, declaredFeatures) {
    const warnings = [];
    if (!requiredFeatures) {
        return warnings;
    }
 ...
 */
export function validateDeepLink(deepLink, toolKey, declaredPermissions, declaredFeatures) {
    const errors = [];
    const warnings = [];
    ...
    const permWarnings = validateDeepLinkPermissions(deepLink.id, deepLink.requiredPermissions, declaredPermissions);
    warnings.push(...permWarnings);
    // Validar features referenciadas
    const featWarnings = validateDeepLinkFeatures(deepLink.id, deepLink.requiredFeatures, declaredFeatures);
 ...
 */
function collectMenuItemIds(items, ids) {
    const duplicates = [];
 ...
 */
export function validateMenuMetadata(contract) {
    const errors = [];
    const warnings = [];
    ...
    if (menu.deepLinks) {
        ...
        for (const deepLink of menu.deepLinks) {
            ...
            const deepLinkValidation = validateDeepLink(deepLink, contract.toolKey, declaredPermissions, declaredFeatures);
 ...
 */
export function generateAllPermissions(toolKey, scopes) {
    const permissions = [];
    const toolKeyStr = toolKey;
    for (const scope of scopes) {
        for (const action of scope.actions) {
            permissions.push(`${toolKeyStr}.${scope.scopeId}.${action.actionId}`);
        }
    }
    return permissions;
}
//# sourceMappingURL=tool-contract.validation.js.map